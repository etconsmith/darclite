...
namespace Darclite.EditorTools
{
    public static class SceneBootstrapper
    {
        ...
        public static void RevertTerrainDetailMeshBakeAxisConversion()
        {
            ...
            Debug.Log("[SceneBootstrapper] Now run 'Darclite/Force Reimport Detail Meshes' to reapply the corrected postprocessor rotation to the remaining detail-brush assets consistently.");
        }
        ...
        private const string TreePrefabPath = "Assets/_Project/Prefabs/Tree.prefab";
        ...
        // Tree.fbx was rebuilt as a proper Chunk_/Static_ hierarchy (Static_TreeStump + a breakable
        // Chunk_TreeTrunk and Chunk_TreeCanopy) instead of the single-mesh Detail Mesh version it
        // used to be, so it goes through the same destructible pipeline as the houses/path: place it
        // by hand instead of painting it, and it takes Lite ability damage and hard-knockback
        // impacts exactly like they do.
        [MenuItem("Darclite/Build Tree Prefab")]
        public static void BuildTreePrefab()
        {
            BuildDestructibleStructurePrefab(TreeModelPath, TreePrefabPath);
        }

        private const string HouseCornerModelPath = "Assets/_Project/Art/Environment/HouseCorner.fbx";
        private const string HouseCornerPrefabPath = "Assets/_Project/Prefabs/HouseCorner.prefab";

        // L-shaped single-story house: a main room plus an attached second room (bedroom), sharing
        // HouseTwoStory/HouseCabin's exact wall/trim/roof colors and gable roof style, with a real
        // interior doorway cut between the two rooms so the whole thing is walkable as one connected
        // space rather than two sealed boxes.
        [MenuItem("Darclite/Build House Corner Prefab")]
        public static void BuildHouseCornerPrefab()
        {
            BuildDestructibleStructurePrefab(HouseCornerModelPath, HouseCornerPrefabPath);
        }

        private const string WheatCropModelPath = "Assets/_Project/Art/Environment/WheatCrop.fbx";
        private const string WheatCropPrefabPath = "Assets/_Project/Prefabs/WheatCrop.prefab";

        // Small wheat patch made of three stalk-bundle chunks, no Static_ base — unlike
        // houses/trees, a destroyed crop is meant to fully disappear rather than leave a
        // permanent remnant behind. Place by hand like the other destructible props.
        [MenuItem("Darclite/Build Wheat Crop Prefab")]
        public static void BuildWheatCropPrefab()
        {
            BuildDestructibleStructurePrefab(WheatCropModelPath, WheatCropPrefabPath);
        }

        private const string FenceModelPath = "Assets/_Project/Art/Environment/Fence.fbx";
        private const string FencePrefabPath = "Assets/_Project/Prefabs/Fence.prefab";

        // Tileable 2m fence segment: the post sits at the segment's local origin (x=0) and the
        // rails span the full 2m width, so placing copies at x=0, 2, 4... lines each post up
        // exactly with the previous segment's rail end, with no gap or doubled post at the seam.
        // Only two chunks (post, rails), both destructible.
        [MenuItem("Darclite/Build Fence Prefab")]
        public static void BuildFencePrefab()
        {
            BuildDestructibleStructurePrefab(FenceModelPath, FencePrefabPath);
        }
